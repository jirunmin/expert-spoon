...
struct scope
{
    ...
    // The total number of byte this scope uses. Aligned to the 16 bytes.
    size_t size;
...
struct string_table_element
{
    ...
    // This is the assembly label that point to the memory
...
struct stack_frame_element
{
    ...
    int type;
    /**
     * The name of the frame element, not a variable name. i.e result value
     */
    const char *name;
...
/**
 * Build tokens for the input string.
 */
...
size_t fixup_sys_unresolved_fixups_count(struct fixup_system *system);
