...
// &a.b.c
struct resolver_entity *resolver_create_new_unray_get_address_entity(struct resolver_process *process, struct resolver_result *result, struct datatype *dtype, struct node *node, struct resolver_scope *scope, int offset)
{
...
{
    struct resolver_entity *entity = resolver_new_entity_for_var_node_no_push(process, var_node, private, offset, resolver_scope_current(process));
    if (!entity)
    {
        return NULL;
    }
...
void resolver_array_brack_set_flags(struct resolver_entity *bracket_entity, struct datatype *dtype, struct node *bracket_node, int index)
{
    ...
    {
        bracket_entity->flags = RESOLVER_ENTITY_FLAG_JUST_USE_STACK;
    }
}
...
struct resolver_entity *resolver_follow_array_bracket(struct resolver_process *resolver, struct node *node, struct resolver_result *result)
{
    ...
    struct resolver_entity_rule rule = {};
    resolver_array_brack_set_flags(array_bracket_entity, &dtype, node, index);
...
struct resolver_entity *resolver_follow_exp_parenthsis(struct resolver_process *resolver, struct node *node, struct resolver_result *result)
{
...
struct resolver_entity *resolver_follow_part_return_entity(struct resolver_process *resolver, struct node *node, struct resolver_result *result)
{
    ...
    case NODE_TYPE_EXPRESSION_PARENTHESES:
        entity = resolver_follow_exp_parenthsis(resolver, node, result);
        break;
...
void resolver_execute_rules(struct resolver_process *resolver, struct resolver_result *result)
{
    ...
    struct resolver_entity *last_process_entity = NULL;
    while (entity)
    {
        if (entity->type == RESOLVER_ENTITY_TYPE_RULE)
        {
            struct resolver_entity *left_entity = resolver_result_pop(result);
            resolver_rule_apply_rules(entity, left_entity, last_process_entity);
            entity = left_entity;
        }

        vector_push(saved_entities, &entity);
        last_process_entity = entity;
...
void resolver_merge_compile_tile(struct resolver_process *resolver, struct resolver_result *result)
{
}
...
struct resolver_result *resolver_follow(struct resolver_process *resolver, struct node *node)
{
    ...
    resolver_merge_compile_tile(resolver, result);
