...
static void expect_keyword(const char *keyword)
{
    ...
    {
        compiler_error(current_process, "Expecting the keyword %s but something else was provided", keyword);
    }
}
...
void parse_exp_normal(struct history *history)
{
    ...
    if (token_peek_next()->type == TOKEN_TYPE_OPERATOR)
    {
        ...
        {
            compiler_error(current_process, "Two operators are expected for a given expression for operator %s", token_peek_next()->sval);
        }
    }
    else
    {
...
void parse_datatype_modifiers(struct datatype *dtype)
{
    ...
    while (token && token->type == TOKEN_TYPE_KEYWORD)
    {
        ...
        else if (S_EQ(token->sval, "__ignore_typecheck"))
        {
...
void parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out)
{
    ...
    {
        compiler_error(current_process, "You not allowed a secondary datatype here for the given datatype %s\n", datatype_secondary_token->sval);
    }
...
{
    // possible bug: need use parser_last_token;
...
void parse_variable(struct datatype *dtype, struct token *name_token, struct history *history)
{
    ...
    if (token_next_is_operator("["))
    {
        ...
        dtype->array.size = array_brackets_calculate(dtype, brackets);
...
void parse_keyword_parenthese_expression(const char *keyword)
{
    // while (1)
...
void parse_switch(struct history *history)
{
    ...
    parse_keyword_parenthese_expression("switch");
...
void parse_do_while(struct history *history)
{
    ...
    parse_body(&var_size, history);
    struct node *body_node = node_pop();
    parse_keyword_parenthese_expression("while");
    struct node *exp_node = node_pop();
    expect_sym(';');
